...
SELECT COUNT(*) AS 'Grunge songs' FROM playlisttrack WHERE PlaylistId = (SELECT PlaylistId FROM playlist WHERE Name = 'Grunge');
...
SELECT * FROM artist WHERE Name LIKE '%Jack%' OR name LIKE '%John%' AND NOT '%Martin%';
...
SELECT BillingCountry, SUM(Total) AS Total FROM invoice GROUP BY BillingCountry HAVING SUM(Total) > 100 ORDER BY SUM(Total) DESC;
...
WHERE customer.Country IN ('USA' , 'Canada' , 'Mexico') GROUP BY employee.FirstName HAVING COUNT(CustomerId) > 6 ORDER BY Employee;
...
SELECT CONCAT(FirstName, ',', LastName), IF(Fax IS NOT NULL, Fax, 'S/he has no Fax') FROM customer WHERE Country = 'USA';

#10. For each employee, show his/her first name, last name, and their age at the time they were hired.
SELECT FirstName, TIMESTAMPDIFF(YEAR, BirthDate, HireDate) AS 'Age at Hiring' FROM employee;